...
local isDrainBleed = LocalPlayer.Name == "drainbleed"

local Library = {
	Themes = {
		Rust = {
			Main = Color3.fromRGB(24, 23, 22),
			Secondary = Color3.fromRGB(51, 50, 50),
			Tertiary = Color3.fromRGB(182, 55, 182),
...
function Library:change_theme(toTheme)
	...
	Library.DisplayName.Text = "welcome back, <font color='rgb(" ..  math.floor(c.R*255) .. "," .. math.floor(c.G*255) .. "," .. math.floor(c.B*255) .. ")'> <b>" .. LocalPlayer.DisplayName .. "</b> </font>"
...
function Library:create(options)
	...
	local game = core:object("TextLabel", {
		AnchorPoint = Vector2.new(0, 1),
		BackgroundTransparency = 1,
		Position = UDim2.new(0, 145, 1, -6),
		Size = UDim2.new(0.2, 0, 0, 10),
		Font = Enum.Font.SourceSans,
		Text = "Hub | "..game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.." [ "..game.PlaceId.." ]",
		Theme = {TextColor3 = "Tertiary"},
		TextSize = 14,
		TextXAlignment = Enum.TextXAlignment.Left
	})
	...
	local chosenImage = "rbxassetid://13192268566"

	if math.random(1, 100) == 1 or isDrainBleed then
		chosenImage = "rbxassetid://13084889497"
	end

	local backgroundImage = content:object("ImageLabel", {
	...
	do
		...
		RunService.Heartbeat:Connect(function(step)
			...
			if counter >= desiredInterval then
				...
				local newString = date:sub(1, date:len()-3)
				local prefixString = "current time: "
				timeDisplay.Text = (prefixString..newString)
			end
		end)
	end
